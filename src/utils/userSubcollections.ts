...
export const initializeUserSubcollections = async (userId: string, retryCount = 0): Promise<void> => {
  try {
    ...
    // 1. Create an empty notifications subcollection (required for app to work)
    // Only create the collection structure, no mock data
    ...
    // 2. Create sessions subcollection for FCM tokens and device tracking
