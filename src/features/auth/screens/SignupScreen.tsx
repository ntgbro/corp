...
const SignupScreen: React.FC = () => {
  ...
  const handleSignup = async (email: string, password: string, name: string) => {
    ...
    // Navigate to email verification screen in all successful cases
    if (result.needsEmailVerification || result.user || !result.error) {
      // Always navigate to email verification screen for successful signup
      navigation.navigate('EmailVerification' as any, { email: email });
      
      // Show alert after navigation to avoid blocking the navigation
      setTimeout(() => {
        if (result.error) {
          Alert.alert(
